@ Test to check the behavior of OS_LeaveOS (0x7c)

@ The program uses the SWI 0x16 to enter Supervisor mode.
@ Then it saves the value of SP, LR and CPSR in the var Context (in the last three words).
@ After that, it overwrites the values of register R0 through R12 with the first thirteen words of the var Context.
@ Then it uses the swi 0x7c to return to user mode.
@ Once you have returned to user mode check register R0-12 maintain their values, registers SP, LR and CPSR have different values that the ones stored in the var Context, and that SPSR is equal to the stored CPSR.
...
Error_Reg: .asciz "Rg00 Rg01 Rg02 Rg03 Rg04 Rg05 Rg06 Rg07 Rg08 Rg09 Rg10 Rg11 Rg12 RgSP RgLR CPSR"
success_text: .asciz "Test ended without errors"
...
_start:
	...
	mrs r1, cpsr
	str r1, [r0]
	@ Bring values from Context (regs 0 - 12) to user mode registers
	ldr r0, =Context
	ldm r0, {r0-r12}
	...
	@ Check the values of registers 0 - r10
	stmdb sp!, {r11-r12} @ Store the value of R11 and R12 for preservation (we need 2 free registers to check the values)
	ldr r11, =Context
	ldr r12, [r11], #4 @ Bring r0 from Context, and advance pointer to r1
	cmp r0, r12
	movne r0, #0
	bne Error
	ldr r12, [r11], #4 @ Bring r1 from Context, and advance pointer to r2
	cmp r1, r12
	movne r0, #1
	bne Error
	...
	moveq r0, #14
	beq Error
	@ Check the values of SPSR & CPSR
	ldr r1, [r0] @ Bring CPSR from Context
	mrs r2, cpsr @ Check the value of the CPSR is different to the one saved in Context
	cmp r1, r2
	moveq r0, #15
	beq Error
NoError:
