@ Test to check the behavior of OS_EnterOS (0x16)
...
_start:
	...
	mov r4, r11
	ldr r1, [r4], #4 @ Bring r11 from Context, and advance pointer to r12
	ldmia sp!, {r11-r12} @ We recover from the stack the stored values of R11 and R12
	cmp r11, r1
	movne r0, #11
	bne Error
	ldr r1, [r4], #4 @ Bring r12 from Context, and advance pointer to sp
	cmp r12, r1
	movne r0, #12
	bne Error

	@ Check the values of sp & lr, they need to be different
	ldr r1, [r4], #4  @ Bring SP from Context, and advance pointer to LR
	cmp r1, sp
	moveq r0, #13
	beq Error
	ldr r1, [r4], #4 @ Bring LR from Context, and advance pointer to CPSR
	cmp r1, lr
	moveq r0, #14
	beq Error
	@ Check the values of SPSR & CPSR
	ldr r1, [r4] @ Bring CPSR from Context
	@ Check the value of the CPSR is different to the one saved in Context
	mrs r2, cpsr
	cmp r1, r2
	moveq r0, #15
	beq Error
	@ Check the SPSR is equal to the old value of the CPSR
	mrs r2, spsr
	cmp r1, r2
	movne r0, #16
	bne Error
